...
export async function fetchAllUsers() {
    ...
    if (user) {
        ...
        const response = await fetch("http://127.0.0.1:8000/staff/profile/all/member", {
            method: "GET",
            headers: {
...
export async function toggleUserApprovedStatus(targetUid) {
    ...
    if (user) {
        const token = await user.getIdToken();
        const response = await fetch('http://127.0.0.1:8000/staff/approved', {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
...
export async function fetchUserProfileById(targetUid) {
    ...
    if (user) {
        ...
        const response = await fetch(`http://127.0.0.1:8000/staff/profile/${targetUid}`, {
            method: 'GET',
            headers: {
                'Authorization': `Bearer ${token}`,
            },
...
export async function updateUserProfile(targetUid, updatedProfile) {
    ...
    if (user) {
        ...
        consttresponse = await fetch('http://127.0.0.1:8000/staff/profile', {
            method: 'PATCH',
            headers: {
                'Content-Type': 'application/json',
                'Authorization': `Bearer ${token}`,
